...
Set up inference environment for vllm-plugin-FL on hardware backends (MetaX, Ascend, Moore Threads, Hygon DCU, etc.).
...
**Hardware backends** (Container Setup sections): MetaX C550 (complete), Ascend 910B (framework), Moore Threads S4000 (framework), Hygon DCU (complete). New backends use the "Adding a New Backend" template.
